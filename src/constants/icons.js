/**
 * This is a list of the icons used in the application.
 * They refer to the 'antd' framework.
 * Please, for more information and check the available icons, visit
 * https://ant.design/components/icon/
 *
 */
export const ADD_IDENTITY = 'user-add';
export const CHANGE_IDENTITY = 'select';
export const BURGER_MENU = 'menu-fold';
export const CAMERA = 'camera';
export const CLAIMS = 'idcard';
export const CLOSE = 'close';
export const DASHBOARD = 'desktop';
export const DELETE_ALL_IDENTITIES = 'usergroup-delete';
export const HISTORICAL = 'database';
export const IDENTITY = 'user';
export const NOTIFICATIONS = 'mail';
